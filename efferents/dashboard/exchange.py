...
from efferents.agents.conference import _rows, _talks, exchange_enabled, measurement_talks
...
def network_evidence() -> dict:
    ...
    for record in Registry().list():
        ...
        try:
            ...
            if not exchange_enabled(root, cfg):
                continue
            talks = _talks(cfg, submission, root) if cfg.conference.enabled else measurement_talks(cfg, root)
            for talk in talks:
    ...
    routed = [{**row, "journal": journal_for_domain(row["domain"]),
               "routing_status": "private journal inbox · not accepted publication"}
