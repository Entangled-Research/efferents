...
def prepare(directory: Path, port: int) -> dict:
    ...
    guide.write_text(
        ...
        "1. Inspect the labs in the network. With the default sample data, each lab has an experiment claim and real CPU runs.\n"
        "2. Follow each lab boundary to its three-reviewer board and journal. Dotted paths are journal subscriptions.\n"
        "3. Choose Start an idea and enter a mathematics question, such as adaptive Simpson integration near a narrow boundary layer.\n"
        "4. Review the displayed starter claim and limitation before choosing Run 3 experiments. The starter is a bounded baseline, not an implementation of any arbitrary idea.\n"
        "5. Inspect the network again. A moving receipt appears only where the saved network includes a persisted accepted-paper delivery; it does not mean agreement or reproduction.\n"
        "6. Open a local lab to inspect its hypothesis, verdict, metrics and SVG evidence. Three evacuation seeds are preliminary; its claim needs twelve.\n\n"
